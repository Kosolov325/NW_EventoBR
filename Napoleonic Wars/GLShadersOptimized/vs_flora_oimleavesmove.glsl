...
void main ()
{
  ...
  vec4 tmpvar_4;
  if ((((
    (((inTexCoord.y <= 0.1) || (inTexCoord.x >= 0.65)) || ((inTexCoord.y >= 0.3) && (inTexCoord.y <= 0.5)))
   || 
    ((inTexCoord.y >= 0.65) && (inTexCoord.y <= 0.8))
  ) || (
    ((inTexCoord.y >= 0.5) && (inTexCoord.y <= 0.65))
   && 
    (((inTexCoord.x >= 0.1) && (inTexCoord.x <= 0.25)) || (inTexCoord.x >= 0.5))
  )) || ((inTexCoord.y >= 0.8) && (
    (inTexCoord.x >= 0.15)
   && 
    (inTexCoord.x <= 0.43)
    ...
    wind_5.x = (sin((
      ((inPosition.z - (inPosition.x * 2.0)) * -1.32)
     + tmpvar_6)) * 0.04515);
    wind_5.y = (cos((
      (inPosition.y * -0.66)
     + tmpvar_6)) * 0.022575);
