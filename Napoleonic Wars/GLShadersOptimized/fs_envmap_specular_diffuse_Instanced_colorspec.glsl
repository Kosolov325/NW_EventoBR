...
void main ()
{
  ...
  specColor_5 = ((tmpvar_3 * dot (texture2D (specular_texture, Tex0).xyz, vec3(0.33, 0.33, 0.33))) * ((1.65 * tmpvar_7) * spec_coef));
  vec4 tmpvar_8;
  tmpvar_8.w = 1.0;
  ...
  fSpecular_6.xyz = (fSpecular_6.xyz + ((specColor_5.xyz * texture2D (env_texture, 
    ((ViewDir - SunLightDir).yz + 1.0)
  ).xyz) * 0.039));
