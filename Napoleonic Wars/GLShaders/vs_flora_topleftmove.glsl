...
VS_OUTPUT_FLORA vs_flora_topleftmove(int PcfMode, float4 vPosition, float4 vColor, float2 tc)
{
	VS_OUTPUT_FLORA Out;
	{
		if ( (tc.y <= 0.25)
		...
		float4 vWorldPos = mul(matWorld,vPosition); //vPosition
...
void main()
{
	...
	VSOut = vs_flora_topleftmove(CURRENT_PCF_MODE, vec4(inPosition, 1.0), inColor0, inTexCoord);
